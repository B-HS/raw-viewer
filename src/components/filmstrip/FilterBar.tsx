...
export const FilterBar: FC = () => {
    ...
    return (
        <div className='flex h-8 shrink-0 items-center gap-2 border-t border-neutral-800 bg-neutral-900 px-3 text-[11px] text-neutral-400'>
            <div className='flex items-center' role='group' aria-label='별점 필터'>
                {RATINGS.map((value) => (
                    <button
                        key={value}
                        type='button'
                        aria-label={`별점 ${value}점 이상`}
            ...
            <span className='text-neutral-700'>|</span>
            <div className='flex items-center gap-1' role='group' aria-label='라벨 필터'>
                {LABELS.map((entry) => (
                    <button
                        key={entry.name}
                        type='button'
                        aria-label={`${entry.name} 라벨`}
