...
export const FilterBar: FC = () => {
    ...
    return (
        <div className='flex h-9 shrink-0 items-center gap-2 border-t border-neutral-800 bg-neutral-900 px-3 text-xs text-neutral-400'>
            <div className='flex items-center' role='group' aria-label={t('filter.ratingFilterAria')}>
                {RATINGS.map((value) => (
                    <button
                        key={value}
                        type='button'
                        title={t('filter.ratingMinAria', { value })}
                        aria-label={t('filter.ratingMinAria', { value })}
                        onClick={() => filter.setMinRating(value)}
                        className={`px-1 py-1 text-base leading-none ${value <= filter.minRating ? 'text-amber-400' : 'text-neutral-600 hover:text-neutral-400'}`}>
                        ★
                    </button>
                ))}
            </div>
            ...
            <button
                type='button'
                title={t('filter.pickOnlyAria')}
                aria-label={t('filter.pickOnlyAria')}
                onClick={() => filter.setFlag('pick')}
                className={`rounded px-2 py-1 text-sm leading-none ${filter.flag === 'pick' ? 'bg-emerald-500/20 text-emerald-300' : 'hover:bg-neutral-800'}`}>
                ⚑
            </button>
            <button
                type='button'
                title={t('filter.rejectOnlyAria')}
                aria-label={t('filter.rejectOnlyAria')}
                onClick={() => filter.setFlag('reject')}
                className={`rounded px-2 py-1 text-sm leading-none ${filter.flag === 'reject' ? 'bg-red-500/20 text-red-300' : 'hover:bg-neutral-800'}`}>
                ⚐
            </button>
            <span className='text-neutral-700'>|</span>
            <div className='flex items-center gap-1.5' role='group' aria-label={t('filter.labelFilterAria')}>
                {LABELS.map((entry) => (
                    <button
                        key={entry.name}
                        type='button'
                        title={t('filter.labelAria', { name: entry.name })}
                        aria-label={t('filter.labelAria', { name: entry.name })}
                        onClick={() => filter.setLabel(entry.name)}
                        style={{ backgroundColor: entry.color }}
                        className={`h-4 w-4 rounded-full ${filter.label === entry.name ? 'ring-2 ring-white ring-offset-1 ring-offset-neutral-900' : 'opacity-60 hover:opacity-100'}`}
                    />
                ))}
            </div>
            <span className='text-neutral-700'>|</span>
            <button
                type='button'
                title={t('filter.editedOnlyTitle')}
